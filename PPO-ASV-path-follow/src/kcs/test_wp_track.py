...
def wp_track(model_name, wind_flag=0, wind_speed=0,wind_dir=0,
    ...
    for policy_name in new_dir_list:
        ...
        plt.legend(["Path Followed","Start Waypoint","Predefined Path"],loc="upper left")
        ...
        if wind_flag == 0 and wave_flag == 0:
            ...
            plt_str = f'saved_models/{model_name}/plots/{traj_str}_{policy_name}_WH{wave_height}_WD{wave_dir}.png'
            plt.savefig(plt_str, dpi=600)
        elif wind_flag == 1 and wave_flag == 1:
            plt.title("$V_{w}^{'}=6$, $Ψ_{w}=90\degree$ ; $W_{h}=3m$, $T_{w}=10s$, $\u03B2_{w}=45\degree$",size = 18)
            plt_str = f'saved_models/{model_name}/plots/{traj_str}_{policy_name}_WH{wind_speed}_WD{wind_dir}_WH{wave_height}_WD{wave_dir}.png'
            plt.savefig(plt_str, dpi=600)
        ...
        savemat('X_RL_PPO_case_VI.mat',{'X_PPO_1':X, 'Y_PPO_1':Y})
