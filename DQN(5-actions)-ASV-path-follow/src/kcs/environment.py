...
class ship_environment(py_environment.PyEnvironment):
    ...
    def __init__(self, train_test_flag=0,
        ...
        # self._observation_spec = array_spec.BoundedArraySpec(shape=(5,), dtype=np.float32, minimum=[-20, -np.pi, 0, -1,-35*np.pi/180],
        #                                                      maximum=[20, np.pi, 50, 1,35*np.pi/180], name='observation')
    ...
    def _step(self, action_no):
        ...
        R3 = -distance * 0.25
        #R4 = -0.25*np.abs(self.action_traj[-1]-self.action_traj[-2])
        reward = R1 + R2 + R3 #+ R4
